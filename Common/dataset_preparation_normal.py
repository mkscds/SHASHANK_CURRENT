...
class CIFAR10PublicDataset(Dataset):
    """Custom CIFAR-10 Dataset that selects only a subset based on a given index sequence."""
    
    def __init__(self, index_sequence=None, subset_size=5000, train=True, transform=None, download=True, seed=42):
        ...
        self.dataset = CIFAR10(root='./data/data_cifar10', train=train, transform=transform_train, download=download)
        
        random.seed(seed)
        # Select a subset of the dataset
        if index_sequence is not None:
            self.index_sequence = index_sequence  # Use provided subset
        else:
            self.index_sequence = random.sample(range(len(self.dataset)), subset_size)  # Random subset

    def __len__(self):
        return len(self.index_sequence)

    def __getitem__(self, idx):
        actual_idx = self.index_sequence[idx]
        image, label = self.dataset[actual_idx]
        return actual_idx, image, label  # Return index for tracking
